...
exp_1a <- exp_1a %>% filter(`Procedure[Block]` == "ExpRun")
...
incomplete <- data.frame(table(exp_1a$subject))
incomplete <- incomplete %>% filter(Freq < 480)
exp_1a_incom <- filter(exp_1a, subject == incomplete$Var1)

exp_1a <- exp_1a %>% 
  filter(!(subject %in% incomplete$Var1))

exp_1a$subject <- droplevels(exp_1a$subject)

# Write data
write_csv(exp_1a, 'data/exp_1a_choice_complete.csv')
write_csv(exp_1a_incom, 'data/exp_1a_choice_incomplete.csv')
